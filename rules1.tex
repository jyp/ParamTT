...
$\CSig i A P$ is another notation for $(A ,_i P)$.
...
\begin{align*}
  ...
  f.i a p &= (f (a,_i p)).i \\
  (a,_i p).i  &= p \\
  (a,_i p).j  &= a.j ,_i \mathsf{swap}( p.j)
\end{align*}

swap works on every second and third arguments, and repeats every four.
...
If we interpret the index of ⊢ as a list (ordered), we never need to swap.
(Additionally, we can never abstract on an (explicitly) colored type, but that was already implicit above.)
