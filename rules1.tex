...
\begin{align*}
  ...
  (a,_i p)_T.i  &= p \\
  (a,_i p)_T.j  &= (a.j ,_i \sw i j T p.j)_{T.j ((a ,_i p) j0)}
\end{align*}
\end{definition}
...
\begin{align*}
  ...
  a'_j : T (j0).i 
\end{align*}
$\sw i j T : T.i.j a a'_i a'_j → T.j.i a a'_j a'_i$

\begin{align*}
  \sw i j {U} R a a'_i a'_j & = R a a'_j a'_i  \\
  \sw i j {(x:A) → B x} f a a'_i a'_j a'' & = \sw i j {B ((a,_ia'_j),_j(a'_i,_ia''))} (f a a'_j a'_i (\sw j i A a'')) \\
  \sw i j {(A ,_i P)_U} & = id \\
  \sw i j {(A ,_j P)_U} & = id \\
  \sw i j {(A ,_k P)_U} & = (\sw i j A ,_k \sw i j {\Pi A P}) & \text {guess}\\
\end{align*}
\end{definition}
