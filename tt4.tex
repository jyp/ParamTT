...
\item We present an extension of Martin-Löf's Logical Framework
  (\fref{sec:syntax}) which internalizes parametricity (as we show in \fref{sec:parametricity})
  ...
  projections to extract the proof.  As we will show in \fref{ex:poly-id},
  ...
  extended calculus, and as explains in
  \cite{bernardy_type-theory_2013}, colors enable nested
...
We do not attempt to explain what lead us to consider Colored Type
...
\item Reynolds associates each type with a
  ...
  (\fref{def:color-erasure}) and can be understood as a
...
\begin{definition}[Typing judgements — à la Tarski]~\\\indent
\fbox{$Γ⊢$}
  \begin{mathpar}
    \inferrule[Empty]{~}{() ⊢ }
...
\fbox{$Γ⊢ A$}
 \begin{mathpar}
    ...
    \inferrule[Swap]{Γ, i:𝕀, j:𝕀, Δ ⊢ A}{Γ, j:𝕀, i:𝕀, Δ  ⊢ A}

    \inferrule[Pi]{Γ ⊢ A \\ Γ,x:A ⊢ B}{Γ ⊢ (x:A) → B}

    \inferrule[Out]{Γ, i:𝕀 ⊢ A \\ Γ ⊢ a : \proj i A}{Γ ⊢ A \op i  a}

    \inferrule[In-Pred]{Γ ⊢ A \\ Γ,x:A ⊢ P}{Γ, i:𝕀 ⊢ \CTimes i {x:A} P}
 \end{mathpar}
...
\fbox{$Γ⊢ a : A$}
  \begin{mathpar}
    ...
    \inferrule[Swap]{Γ, i:𝕀, j:𝕀, Δ ⊢ a : A}{Γ, j:𝕀, i:𝕀, Δ  ⊢ a : A}
    \\
    \inferrule[Lam]{Γ,x:A ⊢ t : B}{Γ ⊢ λx:A.t : (x:A) → B}
 
    \inferrule[App]{Γ ⊢ t : (x:A) → B[x] \\ Γ ⊢ u : A}{Γ ⊢ t u: B[u]}
  \end{mathpar}
  \begin{mathpar}
    \hspace{-1.5em}
    \inferrule[In-Abs]{Γ ⊢ a : \proj i A \\Γ ⊢ p : A \op i a}{Γ, i:𝕀 ⊢ \CP i a p : A}
    \and
    \inferrule[In-Fun]
        {Γ ⊢ t : \projp i {(x:A) → P[x]}\\\\
    ...
    \hspace{-1.5em}
    \\
    \inferrule[Color-Elim]{Γ, i:𝕀 ⊢ a : A}{Γ ⊢ a \param i : A \op i {\proj i a}}
  \end{mathpar}
\end{definition}
...
no variable was introduced after that color (see \eg, \fref{thm:param-2-sym}).
...
\fref{sec:model}; the use of codes enables a presentation à la Tarski,
...
\and
  \inferrule[Pair-Pred]{}{{(\CTimes i {x:A} P[x])} \op i a = P[a]}
    ...
    \inferrule[Surj-Typ]{} {T = \CTimes i {x:{\proj i T}} {(T(i\,j) \op j x)} }
    \\
    \inferrule{}{\El{|A|} = A}
\and
    \inferrule{}{|\El{A}| = A}
\and
    \inferrule[β]{}{(λx:A. t[x]) u = t[u]}
\and
...
\begin{remark}
  ...
  \textsc{Surj-Typ} needs to end with a color binding.
\end{remark}
\begin{remark}
  ...
  naturals (\fref{ex:church-nat}) for instance.
\end{remark}
...
%\todo{more examples: any $f : U → Bool$ is constant,…}
...
using further special symbols $0,1,…,n-1$.
...
\begin{example}
  ...
  %(because $Γ ⊢ (\CTimes i {x:A} {(P\,x)}) \op i a = P\,a$)
...
As seen in \fref{ex:church-nat}, one needs to use
...
\fref{thm:iso-fun}: it turns out that $\fp i t u$ can be combined
...
\begin{definition}[Category $\pI{}$]
  ...
  % and define it as $1_I(i) = i$ for each $i ∈ I$.
...
%  \begin{align*}
%    Γ ∗ 𝕀 ∗ 𝕀(I) =& \{ [ρ,i=0,j=0] \mid ρ ∈ Γ(I) \} ∪
%      \{ [ρ,i=k,j=0] \mid k ∈ I, ρ ∈ Γ(I\backslash\{k\}) \} ∪ \\
%    & \{ [ρ,i=0,j=l] \mid l ∈ I, ρ ∈ Γ(I\backslash\{l\}) \} ∪ \\
%    & \{ [ρ,i=k,j=0] \mid k ∈ I, k ∈ I\backslash\{l\} I, ρ ∈ Γ(I\backslash\{k,l\}) \}
%  \end{align*}
\end{remark}
