...
\begin{remark}
 ...
 $t\param i \param j = t\param j \param i$, to have an
  equality between $A \op i \CP j x y \op j z$ and $A \op j \CP i x z
  \op i y$ rather than an isomorphism, and to have conversion
  rules such as $\CP j a b \param i = \CP j {a \param i}{b \param
    i}$. The answer is that the equation
...
In this section we show how to interpret our type-theory by a presheaf model.
  ...
  Recall \pI{} (\fref{def:pI}), the category of color
  maps.
...
\begin{description}
  \item[\sc Pi.]
    ...
    where each $λ_α$ is a family of elements indexed by any total map $g : J_α ↣ K$:
  ...
  \item[\sc Out.]
    ...
    We can consider the $J,j$-set $P(f',ρι_i)$, and $a(f,ρ)$ a ($J$-)element of
...
\begin{remark}
  Our calculus does not have any base type, but they could interpreted
  by modifying their usual interpretation as a constant presheaf into an
  isomorphic $I$-set.
  For instance, the base type of natural numbers $ℕ$ would be interpreted as
  the $I$-set of $(n_J)_{J ⊆ I}$ where $n_∅ ∈ ℕ$ and $n_J = 0$ for any
  non-empty $J ⊆ I$.
\end{remark}
\todo{We don't interpret terms? Shouldn't we say at least something
  about it? That seems necessary to at the very least give an idea of
  the following proof.}
...
%  $Γf ⊢_J af : Af$).
\end{theorem}
\todo{Proof by induction of on the judgement?}
\begin{theorem}
