...
\begin{remark}
  ...
  \textsc{Surj-Typ} needs to end with the color $i : 𝕀$.
\end{remark}
...
\begin{remark}
  In other words,
  $Γ,x:A⊢$ is interpreted by $(ρ∈Γ) × Aρ$ the cartesian product of the
  interpretation of $Γ⊢$ and $Γ⊢A$, while
  $Γ,i:𝕀⊢$ is interpreted by $Γ ⊗ 𝕀$ the tensor product of the
...
\begin{remark}
  ...
  \begin{align*}
    ...
    \\
    \text{ iff. }& (Qf ,_k v) ∈ (U \op i (X ,_j P))(f,j=k)
    \\
    \text{ iff. }& ((X ,_j P)(f,j=k) ,_l (Qf ,_k v)) ∈ U(l,k)
    \\
  ...
  \begin{align*}
    ...
    \\
    \text{ iff. }& (Pf ,_k v) ∈ (U \op j (X ,_i Q))(f,i=k)
    \\
    \text{ iff. }& ((X ,_i Q)(f,i=k) ,_l (Pf ,_k v)) ∈ U(k,l)
    \\
