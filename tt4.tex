...
\begin{proof}
  ...
  \item Pi.
    ...
    Where $B' = B[\CP i x {\FROM A x'}]$

    (No need to carry around an environment; the proof given by the user "x'" is properly passed to the function. )


    Right to left: $p = (\fp i f {λx. λx'. \FROM B (q x (\TO A x'))}) \param i $
