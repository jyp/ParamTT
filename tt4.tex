...
\item \citet{reynolds_types_1983} associates each type with a
  ...
  many: one for every color. Furthermore these predicates are definable
...
%  and $Δ = Γ, h:A → A, h': (T → T) \op i h$.
  We have $Γ, i:𝕀 ⊢ T$ and
  $$Γ, h:A → A, h': (T → T) \op i h, x : A, x' : P\, x ⊢ \CP i h {h'} : T → T$$
  hence
  ...
  We now have $Γ, i:𝕀 ⊢ \fp i g {g''} : (T → T) → T$ and can finally conclude:
...
\todo{XXX this is not true unless we add an equivalence between $Γ,i:𝕀,j:𝕀$
and $Γ,j:𝕀,i:𝕀$}
\begin{theorem}
