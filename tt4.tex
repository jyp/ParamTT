...
\begin{definition}[Color erasure]~
  \label{def:color-erasure}
  We consider a color $i$,
  and define the term $a(i\,0)$ by structural induction on $a$.
\begin{align*}
  x(i\,0) & = x \\
  U(i\,0) & = U \\
  (λ(x:A).t)(i\,0) &= λ(x:A(i\,0)).t(i\,0) \\
  (t\,u)(i\,0) &= (t(i\,0)) \, (u(i\,0)) \\
  ((x:A)→B)(i\,0) &= (x:A(i\,0))→(B(i\,0)) \\
  {\CP {j} a p}(i\,0)
    &= a
    & \text{if $i = j$} \\
    &= \CP {j} a p
  ...
  {\fp {j} t u}(i\,0)
    ...
    &= \fp {j} t u
    & \text{otherwise} \\
  (A \op {j} a)(i\,0) &= (A(i\,0)) \op {j} (a(i\,0))
    & \text{where $i ≠ j$} \\
  (a · j)(i\,0) &= (a(i\,0)) · j
    & \text{where  $i ≠ j$}
  \end{align*}
  \todo{we need to introduce $α$-equivalence for the two last clauses.
  But we don't want to introduce general color morphisms as we would
  have to change thm 4}
\end{definition}

\begin{theorem}[Color erasure preserves typing]
  If $Γ,i:𝕀  ⊢ a : A$ then the terms $a(i\,0)$ and $A(i\,0)$ are
  defined and $Γ ⊢ a(i\,0) : A(i\,0)$.
\end{theorem}
\begin{proof}
