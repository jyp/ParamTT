...
\begin{itemize}
\item We present a type theory (\fref{sec:syntax}) which internalizes parametricity (\fref{sec:parametricity})
...
\begin{tikzpicture}[node distance=4\baselineskip]%
  \node              (Ii) {$I,i$};
  \node[below of=Ii] (I)  {$I$};
  \node[right of=Ii] (Jj) {$J,i$};
  \node[below of=Jj] (J)  {$J$};

  \draw[->] (Ii) to node[above]  {$f^{ij}$}(Jj);
...
The convertibility of types used in the {\sc Conv} rule is defined
...
\begin{theorem}
  ...
  any proof $a'$ of $A \op i a$ is related to the canonical proof ($a \param
  i$), i.e. formally $A \op i \CP j a {a \param i} \op j a'$.
\end{theorem}
\begin{proof}
...
\begin{remark}
  ...
  Because the $(J,k,l)$-elements $((Xf ,_k Pf) ,_l (Qf ,_k v))$ and
