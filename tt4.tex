...
\begin{definition}
  \begin{align*}
    ⟦A → B⟧ f & = (x:A) → ⟦A⟧ x → ⟦B⟧ (f x)\\
    ⟦U⟧ A & = A → U\\
    ⟦A \op j a ⟧ b &= ⟦A⟧ \CP j {\proj i a} y \op j a \param i \\
    ⟦x⟧ a & = x \op i a
  \end{align*}
\end{definition}
...
\begin{theorem}
For each type $A$, if $A \op i x$ is provable, then so is $⟦A⟧ x$.
\end{theorem}
\begin{proof}
  ...
  \item Pi.
    ...
    \item $p : (A → B) \op i f$
    \end{itemize}
    We prove $q : (x:A) → A \op i x → B \op i (f x)$ with $q x x' = (\CP i f
    p \CP i x {x'}) \param i$.
  ...
  \item Universe.
    ...
    \item $P : U \op i A $
    \end{itemize}
    We prove $Q : A → U$ with $Q x = (\ip i A P) \op i x$.
