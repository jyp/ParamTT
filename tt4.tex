  ...
  \begin{mathpar}
    ...
    \inferrule[In-Fun]
        {Γ ⊢_I t : \projp i {(x:A) → P[x]}\\\\
         Γ ⊢_I u : (x:\proj i A) → (x':A \op i x) → P[\CP i x {x'}] \op i t x}
...
\item $ι_i : I → I,i$ the inclusion: $ι_i(j) = j$ for each $j ∈ I$; and
...
by taking $⟨ρ,x=u⟩_α = ⟨ρ_α,x=u_α⟩$, where and $ρ ∈ Γf$ and
$u ∈ A(f,ρ)$.
The restriction map is defined by $⟨ρ,x=u⟩g = ⟨ρ g, x=ug⟩$.
...
\begin{description}
  ...
  \item[\sc Out.]
    ...
    By the induction hypotheses we get a $(J,j)$-set $P(f^{ij},ρι_i)$,
    and the $J$-element $a(f,ρ)$ belongs to
...
\begin{description}
  ...
  \item[\sc Lam.]
    We define $\app{(λ x:A.b)(f,ρ)_g}{u}$ to be $b(f,⟨ρg,x=u⟩)$
...
\begin{proof}
  ...
  \begin{description}
    \item[\sc Pair-Param.]
      Let $f : I → J$ and $j = \fresh J$.  We have
      \begin{align*}
