...
\Copyright{J-P. Bernardy, T. Coquand and G. Moulin}
...
\item $Γ⊢_I A$  (Is the type $A$ well-formed in $Γ$, assuming the color set $I$?)
\item $Γ⊢_I a : A$  (Does the term $a$ have type $A$ in the context $Γ$, assuming the color set $I$?)
\end{itemize}
...
\item $f^{ij} : I,i → J,j$ the color map such that $f^{ij}(i) = j$
  and $f^{ij}(k) = f(k)$ for all $k ∈ I$
  (if $f : I → J$ with $i ∉ I$ and $j ∉ J$).
\end{itemize}
...
\begin{theorem}
  ...
  is related to the canonical proof ($a \param i$), i.e. formally $A
...
satisfying $u1 = u$ and $(uf)g = u(fg)$. We use a refined presheaf on
\pI{} by requiring two further conditions:
%
\begin{enumerate}
...
\begin{description}
  \item[\sc Pi.]
    ...
    such that
    $\app{λ_{αg}}{u} h = \app{λ_{α gh}}{uh}$ for $g : J ↣ K$ total and
    for \emph{any} $h : K → L$.
    ...
    each element as a tuple, which can be done naturally by repartioning
...
\begin{proof}
  ...
  We only show the conversion rules \textsc{Pair-Param} and
...
\begin{remark}
  ...
  \begin{align*}
    ...
    \\
    \text{ iff. }& ((Xf ,_k Pf) ,_l (Qf ,_k v)) ∈ Uf^{ik,jl} & \text{Because neither $i$ or $j$ occur in $U$}
  \end{align*}
  ...
  Let $w  = ((Xf ,_k Pf) ,_l (Qf ,_k v))$
  and $w' = ((Xf ,_k Qf) ,_l (Pf ,_k v))$.
  We have $w(l\,0) = (Xf ,_k Pf)$ and $w'(l\,0) = (Xf ,_k Qf)$.
  But an element of $U(k,l)$ is a tuple $(A_∅, A_k, A_l, A_{kl})$
  $(Xf ,_k Pf) ∈ A_k $ iff. $(Xf ,_k Qf) ∈ A_k$ does not hold for
  abitrary $P$ and $Q$.
\end{remark}


\begin{theorem}[Validity]
  For any $f : I → J$ and any $ρ ∈ Γf$,\\
    if $Γ ⊢_I a : A$ then $a(f,ρ) ∈ A(f,ρ)$.
...
\begin{proof}
  ...
  \begin{description}
    ...
    \item[\sc Color-Elim.]
      ...
      ∈ T(f^{ij},ρι_j)$
      where $j = \fresh J$.
      By induction hypothesis $a(f^{ij},ρι_j) ∈ T(f^{ij},ρι_j)$, and because it is a
      $(J,j)$-element we have
